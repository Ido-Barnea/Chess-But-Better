...
import { Piece, Pawn, Bishop, Knight, Rook, Queen, King, Square } from "./pieces";
...
function actOnTurn(draggedPiece: Piece | undefined, target: Piece | Square | undefined) {
...
function castle(kingPiece: Piece,  targetSquare: Square) {
    const possibleRooks = pieces.filter((piece) => {
        return piece.player === getCurrentPlayer() &&
            !piece.hasMoved &&
            piece.name === 'Rook';
    });
    ...
    const rookPieceTargetPosition: [number, number] = [isKingsideCastling ? targetSquare.position[0] - 1 : targetSquare.position[0] + 1, kingPiece.position[1]];
